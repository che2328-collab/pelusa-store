      ...
      <Modal open={!!sel} onClose={() => setSel(null)} title={`Pedido ${sel?.id}`} wide>
        {sel && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {(sel.status === "activo" || sel.status === "pendiente_pago") && (
              <div style={{ background: GOLD_DIM, border: `1px solid ${GOLD}33`, borderRadius: 12, padding: 16 }}>
                <div style={{ fontSize: 12, color: GOLD, fontWeight: 700, marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                  🏦 Datos para tu transferencia / abono
                </div>
                <BankInfoDisplay />
  ...
  return (
    <div>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal === "new" ? "Nuevo Producto" : "Editar Producto"} wide>
        ...
        {/* Tallas para calzado */}
        {(form.category || "").toLowerCase() === "calzado" && (() => {
          const allSizes = [];
          for (let s = 2; s <= 32; s += 0.5) allSizes.push(s);
                ...
                {allSizes.map(size => {
                  ...
                  return (
                    <button key={size} onClick={() => toggle(size)} style={{
                      ...
                      {Number.isInteger(size) ? size : size.toFixed(1)}
                    </button>
                  );
                })}
              </div>
...
const AdminOrders = ({ orders, setOrders, users, products }) => {
  ...
  const createOrder = () => {
    ...
    setCreatedOrder({ ...order, clientPhone: user?.phone || "" });
  };
  ...
  return (
    <div>
      ...
      <Modal open={!!sel} onClose={() => setSel(null)} title={`Pedido ${sel?.id}`} wide>
        {sel && (
          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            {/* ── PAGO CONTADO ── */}
            {sel.type === "contado" && sel.status === "pendiente_pago" && (() => {
              const clientUser = users.find(u => u.id === sel.userId);
              const clientPhone = sel.customerPhone || clientUser?.phone || "";
              const msgLiquidado = `Hola ${sel.customerName} 🎉\n\n¡Tu pedido *${sel.id}* ha sido *LIQUIDADO* en *Pelusa Store*! ✅\n\n💵 Monto pagado: *${fmt(sel.total)} MXN*\n📅 Fecha de pago: ${today()}\n\n¡Muchas gracias por tu pago puntual! 🙏 Seguimos a tus órdenes.`;
              return (
                <div style={{ background: C.greenDim, border: `1px solid ${C.green}44`, borderRadius: 12, padding: 16 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: C.green, marginBottom: 6 }}>
                    💵 Registrar pago de contado
                  </div>
                  {sel.payDueDate && (
                    <div style={{ fontSize: 12, color: C.muted, marginBottom: 12 }}>
                      📅 Fecha límite acordada: <strong style={{ color: sel.payDueDate < today() ? C.red : C.yellow }}>{sel.payDueDate}</strong>
                      {sel.payDueDate >= today() && <span style={{ color: C.green, marginLeft: 6 }}>✓ Pago anticipado</span>}
                    </div>
                  )}
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <Btn variant="success" onClick={async () => {
                      ...
                      ✅ Registrar pago completo — {fmt(sel.total)}
                    </Btn>
                    <a href={buildWALink(msgLiquidado, clientPhone)} target="_blank" rel="noreferrer" style={{
                      display: "inline-flex", alignItems: "center", gap: 6,
                      background: "#25D36622", border: "1px solid #25D36644", color: "#25D366",
                      borderRadius: 8, padding: "9px 16px", fontSize: 13, fontWeight: 600, textDecoration: "none",
                    }}>
                      <svg width={14} height={14} viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                      📲 Avisar pago liquidado
                    </a>
                  </div>
                  <div style={{ fontSize: 11, color: C.muted, marginTop: 8 }}>
                    💡 Primero registra el pago y luego manda el aviso al cliente por WhatsApp
                  </div>
                </div>
              );
            })()}

            {sel.type === "contado" && sel.status === "pagado" && (
              <div style={{ background: C.greenDim, border: `1px solid ${C.green}44`, borderRadius: 10, padding: 14, fontSize: 13, color: C.green, fontWeight: 600 }}>
                ✅ Pedido liquidado el {sel.abonos?.slice(-1)[0]?.date || sel.date}
              </div>
            )}
      ...
      {/* Modal pedido creado — aviso WA */}
      <Modal open={!!createdOrder} onClose={() => setCreatedOrder(null)} title="✅ Pedido creado">
        {createdOrder && (() => {
          const itemsText = createdOrder.items.map(i =>
            `• ${i.name}${i.selectedSize ? ` (Talla ${i.selectedSize})` : ""} × ${i.qty} — ${fmt(i.price * i.qty)}`
          ).join("\n");
          const esCredito = createdOrder.type === "credito";
          const abono = esCredito ? weekAbono(createdOrder.total, createdOrder.semanasTotal) : 0;
          const msg = `Hola ${createdOrder.customerName} 👋\n\n*Pelusa Store* te informa que se ha registrado un nuevo pedido a tu nombre 🛍️\n\n*Pedido:* ${createdOrder.id}\n*Fecha:* ${createdOrder.date}\n\n${itemsText}\n\n💰 *Total: ${fmt(createdOrder.total)}*\n${esCredito
            ? `📅 Modalidad: Crédito semanal\n💵 Abono semanal: ${fmt(abono)} × ${createdOrder.semanasTotal} semanas${createdOrder.firstPayDate ? `\n📅 Primer abono: ${createdOrder.firstPayDate}` : ""}`
            : `💵 Modalidad: Contado${createdOrder.payDueDate ? `\n📅 Fecha límite de pago: ${createdOrder.payDueDate}` : ""}`
          }${createdOrder.note ? `\n\n📝 Nota: ${createdOrder.note}` : ""}\n\n¡Gracias por tu preferencia! 🙌`;

          return (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ background: C.greenDim, border: `1px solid ${C.green}44`,
                borderRadius: 10, padding: 14, fontSize: 13, color: C.green, fontWeight: 600 }}>
                ✅ Pedido <strong>{createdOrder.id}</strong> creado para <strong>{createdOrder.customerName}</strong>
              </div>

              {/* Resumen */}
              <div style={{ background: C.bg, borderRadius: 10, padding: 14, fontSize: 13 }}>
                {createdOrder.items.map((i, idx) => (
                  <div key={idx} style={{ display: "flex", justifyContent: "space-between",
                    padding: "6px 0", borderBottom: `1px solid ${C.border}` }}>
                    <span>{i.name}{i.selectedSize ? ` — Talla ${i.selectedSize}` : ""} × {i.qty}</span>
                    <span style={{ fontFamily: "JetBrains Mono", color: C.accent }}>{fmt(i.price * i.qty)}</span>
                  </div>
                ))}
                <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700,
                  paddingTop: 10, fontSize: 15 }}>
                  <span>Total</span>
                  <span style={{ fontFamily: "JetBrains Mono", color: GOLD }}>{fmt(createdOrder.total)}</span>
                </div>
                {esCredito && (
                  <div style={{ fontSize: 12, color: C.muted, marginTop: 6 }}>
                    📅 {abono && `${fmt(abono)}/sem × ${createdOrder.semanasTotal} semanas`}
                    {createdOrder.firstPayDate && ` · Primer abono: ${createdOrder.firstPayDate}`}
                  </div>
                )}
                {!esCredito && createdOrder.payDueDate && (
                  <div style={{ fontSize: 12, color: C.muted, marginTop: 6 }}>
                    📅 Fecha límite de pago: {createdOrder.payDueDate}
                  </div>
                )}
              ...
              {/* Botón WA */}
              <a href={buildWALink(msg, createdOrder.clientPhone)} target="_blank" rel="noreferrer"
                style={{
                  display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
                  background: "#25D366", color: "#fff", borderRadius: 12,
                  padding: "14px 20px", fontSize: 15, fontWeight: 700, textDecoration: "none",
              ...
              <div style={{ fontSize: 11, color: C.muted, textAlign: "center" }}>
                El mensaje ya incluye el detalle completo del pedido
              </div>

              <Btn variant="ghost" onClick={() => setCreatedOrder(null)}>Cerrar</Btn>
            </div>
          );
        })()}
      </Modal>
